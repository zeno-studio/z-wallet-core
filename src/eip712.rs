...
impl EIP712 {
    ...
    /// Keep signature stable for easy replacement, now returns WalletError.
    pub fn hash_eip712_message(json: &str) -> Result<B256, WalletError> {
        let value: Value = serde_json::from_str(json).map_err(|_| WalletError::JsonParseError)?;

        let domain = value.get("domain").ok_or(WalletError::MissingDomain)?;
        let types = value.get("types").ok_or(WalletError::MissingTypes)?;
        let primary_type = value
            .get("primaryType")
            .and_then(|v| v.as_str())
            .ok_or(WalletError::MissingPrimaryType)?;
        let message = value.get("message").ok_or(WalletError::MissingMessage)?;
    ...
    pub fn struct_hash(types: &Value, struct_name: &str, data: &Value) -> Result<B256, WalletError> {
        ...
        let fields = types
            ...
            .ok_or(WalletError::TypeFieldsNotArray)?;

        for field in fields {
            let name = field
                .get("name")
                .and_then(|v| v.as_str())
                .ok_or(WalletError::FieldMissingName)?;
            let typ = field
                .get("type")
                .and_then(|v| v.as_str())
                .ok_or(WalletError::FieldMissingType)?;
            let value = data.get(name).ok_or(WalletError::MissingFieldValue)?;
            let encoded_value =
                Self::encode_value(types, typ, value).map_err(|_| WalletError::JsonParseError)?;
    ...
    fn type_hash(types: &Value, primary_type: &str) -> Result<B256, WalletError> {
    ...
    fn build_type_str(types: &Value, typ: &str) -> Result<String, WalletError> {
        let fields = types
            .get(typ)
            .and_then(|v| v.as_array())
            .ok_or(WalletError::TypeFieldsNotArray)?;
    ...
    fn collect_types(
        ...
        if path.contains(&typ_owned) {
            return Err(WalletError::CycleDetected);
        }
    ...
    fn encode_value(types: &Value, typ: &str, value: &Value) -> Result<B256, WalletError> {
    ...
    fn encode_value_parsed(types: &Value, parsed: &ParsedType, value: &Value) -> Result<B256, WalletError> {
        // If there are array dimensions -> current level is array
        if !parsed.array_dims.is_empty() {
            // value must be array
            let arr = value.as_array().ok_or(WalletError::UnsupportedType)?;
    ...
    fn encode_atomic(base: &str, value: &Value) -> Result<B256, WalletError> {
        match base {
            "address" => {
                let addr_str = Self::value_to_string(value)?;
                let addr_clean = addr_str.strip_prefix("0x").unwrap_or(&addr_str);
                let addr_bytes = hex_decode(addr_clean).map_err(|_| WalletError::InvalidAddress)?;
            ...
            "bool" => {
                let b = match value {
                    ...
                    _ => return Err(WalletError::InvalidNumber),
                };
            ...
            t if t.starts_with("uint") => {
                ...
                let num = U256::from_str(&num_str).map_err(|_| WalletError::UnsupportedType)?;
            ...
            t if t.starts_with("int") => {
                ...
                let num_str = Self::value_to_string(value)?;
                let num = I256::from_str(&num_str).map_err(|_| WalletError::UnsupportedType)?;
                if bits < 256 {
                    let abs = num.unsigned_abs();
                    let limit = if bits == 0 {
                        return Err(WalletError::InvalidTypePrefix);
                    } else {
                        U256::ONE << (bits - 1)
                    };
                    ...
                    if overflow {
                        return Err(WalletError::ValueOverflow);
                    }
                }
                Ok(B256::from_slice(&num.to_be_bytes::<32>()))
            }
            t if t.starts_with("bytes") && t != "bytes" => {
                ...
                let b = hex_decode(s.strip_prefix("0x").unwrap_or(&s))
                    .map_err(|_| WalletError::InvalidBytesHex)?;
                if b.len() > bytes_len {
                    return Err(WalletError::ValueOverflow);
                }
                let mut padded = [0u8; 32];
    ...
    fn parse_bits(typ: &str, prefix: &str) -> Result<usize, WalletError> {
        let suff = typ.strip_prefix(prefix).ok_or(WalletError::InvalidTypePrefix)?;
        let bits = suff.parse::<usize>().map_err(|_| WalletError::InvalidTypePrefix)?;
        match prefix {
            "bytes" => {
                if bits == 0 || bits > 32 {
                    return Err(WalletError::InvalidTypePrefix);
            ...
            _ => return Err(WalletError::InvalidTypePrefix),
        }
        Ok(bits)
    }

    /// Convert JSON value to string for numeric/string/hex processing.
    fn value_to_string(value: &Value) -> Result<String, WalletError> {
        match value {
    ...
    fn hash_domain_fallback(domain: &Value) -> Result<B256, WalletError> {
        ...
        if let Some(v) = domain.get("name") {
            match v {
                ...
                _ => return Err(WalletError::DomainFallbackInvalid),
            }
        } else {
        ...
        if let Some(v) = domain.get("chainId") {
            ...
            let num = U256::from_str(&s).map_err(|_| WalletError::DomainFallbackInvalid)?;
        ...
        if let Some(v) = domain.get("verifyingContract") {
            ...
            let addr_bytes = hex_decode(addr_clean).map_err(|_| WalletError::DomainFallbackInvalid)?;
            if addr_bytes.len() != 20 {
                return Err(WalletError::DomainFallbackInvalid);
            }
            let mut padded = [0u8; 32];
            padded[12..].copy_from_slice(&addr_bytes);
        ...
        if let Some(v) = domain.get("salt") {
            let s = Self::value_to_string(v)?;
            let b = hex_decode(s.strip_prefix("0x").unwrap_or(&s))
                .map_err(|_| WalletError::DomainFallbackInvalid)?;
            if b.len() != 32 {
                return Err(WalletError::DomainFallbackInvalid);
            }
